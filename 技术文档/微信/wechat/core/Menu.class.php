...
class Menu{
     ...
     * @return bool
     */
    public static function setMenu($menuList, $appid=null){
        // var_export($menuList);die;
        ...
        foreach($menuList as $key=>$menu){
            ...
            if(@$menu['type'] == 'view'){
                ...
                list($_appid, $path) = explode('|', $menu['code']);
                $menuList[$key]['appid']    = $_appid;
                $menuList[$key]['pagepath'] = $path;
            ...
            if(isset($menu['sub_button'])){
                ...
                foreach($menu['sub_button'] as $k=>$son){
                    ...
                    if($son['type'] == 'view'){
                        ...
                        list($_appid, $path) = explode('|', $son['code']);
                        //$menuList[$key]['sub_button'][$k]['key']    = $_appid;
                        $menuList[$key]['sub_button'][$k]['appid']    = $_appid;
                        $menuList[$key]['sub_button'][$k]['pagepath'] = $path;
                        $menuList[$key]['sub_button'][$k]['url'] = 'http://about.12301.cc';
                    } else{
...
//        var_export($result);
    ...
    public function SetMenuOrigin($json)
    {
        $accessToken = AccessToken::getAccessToken();
        $url = 'https://api.weixin.qq.com/cgi-bin/menu/create?access_token='.$accessToken;
        $result = Curl::callWebServer($url, $json, 'POST');
//        var_export($result);
        if($result['errcode'] == 0){
            return true;
        }
     ...
     */
    public static function getMenu($appid=null){
        //获取ACCESS_TOKEN
//        $accessToken = AccessToken::getAccessToken();
        if (!is_null($appid)) {
            $accessToken = OpenAccessToken::getAccessToken($appid);
