...
class Curl {
                                         ...
                                         $post_json = false)
    {
        ...
        if ($method == 'get') {
            ...
            $ret = self::_httpPost($queryUrl, $param, $is_urlcode, $post_json);
        }
        if(!empty($ret)){
            if($is_json){
    ...
    private static function _httpPost($url, $query = array(), $is_urlcode = true, $post_json = false)
    {
        if (is_array($query)) {
