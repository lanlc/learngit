...
class Popularize
{
     ...
     * )
     */
    public static function createTicket($type, $expireSeconds, $sceneId, $appid = '')
    {
        $appid = $appid ?: WECHAT_APPID;
        $queryUrl = 'https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token=' . OpenAccessToken::getAccessToken($appid);
        $queryAction = 'POST';
        $template = array();
        if ($type == 1) {
...
//            $template['action_name'] = 'QR_LIMIT_STR_SCENE';
        ...
        $template['action_info']['params'] = 1111;
//        print_r($template);
    ...
    {
        $queryUrl = 'https://api.weixin.qq.com/cgi-bin/shorturl?access_token=' . OpenAccessToken::getAccessToken(WECHAT_APPID);
        $queryAction = 'POST';
        $template = array();
        $template['long_url'] = $longUrl;
        $template['action'] = 'long2short';
        return Curl::callWebServer($queryUrl, $template, $queryAction, true, false, true);
    }
}