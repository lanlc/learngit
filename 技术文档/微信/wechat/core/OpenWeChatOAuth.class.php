...
//        $redirect_uri = $redirect_uri;
        ...
        $url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='
            .$appid.'&redirect_uri='.$redirect_uri.'&response_type='.$response_type
     ...
     * @return bool|mixed
     */
    public static function getAccessTokenAndOpenId($appid, $code){
        //构造请求微信接口的URL
        $access_token = OpenAccessToken::getOpenAccessToken();
        $ret = Curl::callWebServer(self::openIdUrl($appid, $code, $access_token));
        if ($ret['errcode']==40094) {
            $accessTokenArr = OpenAccessToken::OpenAccessToken();
            $access_token   = $accessTokenArr['component_access_token'];
            $ret = Curl::callWebServer(self::openIdUrl($appid, $code, $access_token));
        } elseif ($ret['errcode'] == 40001) {
            $access_token = OpenAccessToken::getOpenAccessToken(true);
            $ret = Curl::callWebServer(self::openIdUrl($appid, $code, $access_token));
        }
        return $ret;
    }
    private static function openIdUrl($appid, $code, $access_token)
    {
        $url = 'https://api.weixin.qq.com/sns/oauth2/component/access_token?appid='
            .$appid.'&code='.$code.'&grant_type=authorization_code&component_appid='.OPEN_WECHAT_APPID
     ...
     */
    public static function refreshToken($appid, $refreshToken){
        $queryUrl = 'https://api.weixin.qq.com/sns/oauth2/component/refresh_token?appid='.$appid
            .'&grant_type=refresh_token&component_appid='.OPEN_WECHAT_APPID
            .'&component_access_token='.AccessToken::getOpenAccessToken().'&refresh_token='.$refreshToken;
        $queryAction = 'GET';
