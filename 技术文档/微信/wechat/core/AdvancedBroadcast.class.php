...
class AdvancedBroadcast{
     ...
     * @return mixed array("errcode"=>0, "errmsg"=>"success","msg_id"=>34182} 正常是errcode为0
     */
    public static function sentTextByOpenId($toUserList, $content, $appid=null){
        if ($appid) {
            $access_token = OpenAccessToken::getAccessToken($appid);
        } else {
            $access_token = AccessToken::getAccessToken();
        }
        $queryUrl = 'https://api.weixin.qq.com/cgi-bin/message/mass/send?access_token='.$access_token;
        $queryAction = 'POST';
//        $template = array();
//        $template['touser'] = $toUserList;
//        $template['text']['content'] = $content;
//        $template['msgtype'] = 'text';
        //$template = json_encode($template);
        if (count($toUserList)==1) $toUserList[] = '1';
        $toUserList = json_encode($toUserList);
        $template = <<<JSON
{
    "touser":$toUserList,
    "msgtype":"text",
